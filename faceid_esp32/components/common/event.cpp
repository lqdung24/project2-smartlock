...
void set_timeout(uint32_t delay_ms, timeout_cb_t callback_func, void *arg)
{
    ...
    ESP_ERROR_CHECK(esp_timer_start_once(reusable_timer_handle, delay_ms * 1000ULL));

    ESP_LOGI(TAG, "Đã hẹn giờ %lu ms. Hệ thống tiếp tục làm việc khác...", delay_ms);
}
...
static void enroll_done_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
...
static void mqtt_connected_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
...
static void get_value(const char *buf,
                      const char *key,
                      char *out,
                      size_t out_size,
                      char terminator);
 ...
 */
static void config_save_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
 ...
 */
static void config_log_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    if (id != APP_EVENT_CONFIG_RECEIVED)
    {
        return;
...
static bool regis_to_server(app_config_t cfg)
{
    ...
    snprintf(body,
             sizeof(body),
             "{\"hardwareId\":\"%s\",\"provisionToken\":\"%s\"}",
             hardwareId,
             provison_token);
    sprintf(server, "http://%s:%d/device/confirm", cfg.server_host, cfg.server_port);
    // printf("URL : %s\n", server);
    // printf("BODY: %s\n", body);
    if (http_post(server, body, &response))
    {
        ESP_LOGI(TAG, "Status: %d, Body: %s, len %d",
                 response.status_code, response.response_body, response.response_len);
        char port[8];
        get_value(response.response_body, "\"mqttToken\":\"",
                  cfg.mqtt_token, sizeof(cfg.mqtt_token), '\"');
        get_value(response.response_body, "\"mqttHost\":\"",
                  cfg.mqtt_host, sizeof(cfg.mqtt_host), '\"');
        get_value(response.response_body, "\"mqttPort\":\"",
                  port, sizeof(port), '\"');
...
static void config_connect_wifi(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
...
static void config_mqtt_connected(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
...
app_config_t parse_to_config_t(uint8_t *buf)
{
    ...
    get_value((char *)buf, "wifi_ssid=", cfg.wifi_ssid,
              sizeof(cfg.wifi_ssid), ';');

    get_value((char *)buf, "wifi_pass=", cfg.wifi_pass,
              sizeof(cfg.wifi_pass), ';');

    get_value((char *)buf, "server_host=", cfg.server_host,
              sizeof(cfg.server_host), ';');

    get_value((char *)buf, "server_port=", port_str,
              sizeof(port_str), ';');
